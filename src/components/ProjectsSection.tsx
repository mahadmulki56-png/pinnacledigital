  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-8 sm:gap-16 perspective-container">
          {PROJECTS.map((project, idx) => (
            <div
              key={project.number}
              id={`project-card-${project.number}`}
              className="relative rounded-2xl sm:rounded-[36px] glass-card card-project-depth p-5 sm:p-8 lg:p-12 overflow-hidden border border-white/10 light:border-black/10 group"
            >
                ...
                <div className="lg:col-span-6 flex flex-col justify-between h-full">
                  <div>
                    ...
                    <div className="flex items-center justify-between gap-4 mb-3 sm:mb-4">
                      ...
                      <span className="font-display font-extrabold text-4xl sm:text-6xl lg:text-7xl text-white/[0.08] light:text-black/[0.08] select-none">
                        {project.number}
                      </span>
                    </div>

                    <h3 className="font-display font-bold text-[clamp(1.35rem,3vw,2.25rem)] text-[#f4f7f4] light:text-[#0d140f] tracking-tight mb-3 sm:mb-4">
                      {project.title}
                    </h3>

                    <p className="text-xs sm:text-sm md:text-base text-[#a0ada3] light:text-[#4a594e] leading-relaxed mb-4 sm:mb-6 max-w-xl">
                      {project.description}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-6 sm:mb-8">
                      {project.tags.map((tag) => (
                        <span
                          key={tag}
                          className="px-2.5 sm:px-3 py-1 rounded-full text-[11px] sm:text-xs font-medium bg-white/[0.04] light:bg-black/[0.04] border border-white/10 light:border-black/10 text-[#f4f7f4] light:text-[#0d140f]"
                        >
                          {tag}
                        </span>
                      ))}
